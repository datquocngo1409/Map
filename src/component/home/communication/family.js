...
export default class Family extends Component {
  ...
  async getFamily() {
    ...
    const familyMembersString = await AsyncStorage.getItem('familyMembers');
    const familyMembers = JSON.parse(familyMembersString);
    this.setState({users: familyMembers});
  }
      ...
      <LinearGradient
        ...
        <View style={styles.header}>
          ...
          <View style={styles.tittleContainer}>
            <Text style={styles.headerTittleText}>YOUR FAMILY MEMBERS</Text>
          </View>
          <View style={styles.refresh}>
            <TouchableHighlight
              style={[styles.goHome]}
              onPress={() => this.getFamily()}>
        ...
        <View style={styles.content}>
                ...
                <TouchableOpacity>
                  <View style={styles.box}>
                    ...
                    <View style={styles.addButtonContainer}>
                      ...
                      <TouchableOpacity
                        onPress={() => this.unFamily(item, item.id)}>
          ...
          <View style={styles.buttonBottom}>
            <TouchableHighlight
              ...
              <Text style={styles.loginText}>ADD NEW FAMILY MEMBERS</Text>
            </TouchableHighlight>
            <RBSheet
              ref={(ref) => {
                this.RBSheet = ref;
  ...
  async unFamily(item, id) {
    const userId = await AsyncStorage.getItem('userId');
    const token = await AsyncStorage.getItem('token');
    api.unFamily(token, userId, id);
    const result = await AsyncStorage.getItem('unFamilySuccess');
    if (result === 'true') {
      this.state.users.splice(this.state.users.indexOf(item), 1);
      await AsyncStorage.setItem('unFamilySuccess', '');
      api.getFamilyMembers(token, userId);
      api.getRecommendFamilyMembers(token, userId);
      this.forceUpdate();
    }
