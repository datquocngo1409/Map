...
import NewColleague from './new-colleague';
...
export default class Colleague extends Component {
  ...
  async getColleague() {
    ...
    const colleagues = JSON.parse(colleaguesString);
    this.setState({users: colleagues});
  }
      ...
      <LinearGradient
        ...
        <View style={styles.header}>
          ...
          <View style={styles.tittleContainer}>
            <Text style={styles.headerTittleText}>YOUR COLLEAGUE</Text>
          </View>
          <View style={styles.refresh}>
            <TouchableHighlight
              style={[styles.goHome]}
              onPress={() => this.getColleague()}>
        ...
        <View style={styles.content}>
                ...
                <TouchableOpacity>
                  <View style={styles.box}>
                    ...
                    <View style={styles.addButtonContainer}>
                      ...
                      <TouchableOpacity
                        onPress={() => this.unColleague(item, item.id)}>
          ...
          <View style={styles.buttonBottom}>
            <TouchableHighlight
              ...
              <Text style={styles.loginText}>ADD NEW COLLEAGUE</Text>
            </TouchableHighlight>
            <RBSheet
              ref={(ref) => {
                this.RBSheet = ref;
  ...
  async unColleague(item, id) {
    const userId = await AsyncStorage.getItem('userId');
    const token = await AsyncStorage.getItem('token');
    api.unColleague(token, userId, id);
    const result = await AsyncStorage.getItem('unColleagueSuccess');
    if (result === 'true') {
      this.state.users.splice(this.state.users.indexOf(item), 1);
      await AsyncStorage.setItem('unColleagueSuccess', '');
