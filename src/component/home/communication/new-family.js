...
export default class NewFamily extends Component {
  ...
  constructor(props) {
    super(props);
    this.getRecommendFamilyMembers();
  }

  async getRecommendFamilyMembers() {
    const friendsString = await AsyncStorage.getItem('familyMembersRecommend');
    ...
    return (
      <LinearGradient
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            return (
              <TouchableOpacity onPress={() => this.addFamily(item, item.id)}>
  ...
  async addFamily(item, id) {
    const userId = await AsyncStorage.getItem('userId');
    const token = await AsyncStorage.getItem('token');
    api.addFamily(token, userId, id);
    const result = await AsyncStorage.getItem('addedFamilySuccess');
    if (result === 'true') {
      await AsyncStorage.setItem('addedFamilySuccess', '');
